    ...
    <Card>
      <Grid container>
        <Grid
          ...
          xs={12}
          lg={12}
          style={{ maxHeight: "32px", marginBottom: "16px" }}
        >
...
const AddressBox = () => {
  ...
  useEffect(() => {
    if (pickupAddress !== null) {
      const { longitude: lng, latitude: lat } = pickupAddress;
      drawMarker({ type: PICKUP, lat, lng });
    }
  }, [pickupAddress]);
  useEffect(() => {
    if (dropOffAddress !== null) {
      const { longitude: lng, latitude: lat } = dropOffAddress;
